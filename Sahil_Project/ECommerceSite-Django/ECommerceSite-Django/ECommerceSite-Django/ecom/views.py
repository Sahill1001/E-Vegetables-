...
def add_to_cart_view(request,pk):
    ...
    if 'product_ids' in request.COOKIES:
        product_ids = request.COOKIES['product_ids']
        product_qts = request.COOKIES['product_qts']
        if product_ids=="":
            product_ids=str(pk)
            product_qts=str(1)
        else:
            product_ids=product_ids+"|"+str(pk)
            product_qts=product_qts+"|"+str(1)
        response.set_cookie('product_ids', product_ids)
        response.set_cookie('product_qts', product_qts)
    else:
        response.set_cookie('product_ids', pk)
        response.set_cookie('product_qts', "1")
...
def customer_address_view(request):
    ...
    if request.method == 'POST':
        ...
        if addressForm.is_valid():
            ...
            response = render(request, 'ecom/payment.html',{'products':products, 'total':total})
            response.set_cookie('email',email)
            response.set_cookie('mobile',mobile)
            response.set_cookie('address',address)
            return response
    return render(request,'ecom/customer_address.html',{'customer':customer,'addressForm':addressForm,'product_in_cart':product_in_cart,'product_count_in_cart':product_count_in_cart})
...
def payment_success_view(request):
    ...
    items = json.loads(request.GET.get('cart'))
    print(items)
    customer=models.Customer.objects.get(user_id=request.user.id)
    products=None
    email=None
    ...
    for product in products:
        quantity = items[str(product.id)]
        print(quantity)
        models.Orders.objects.get_or_create(customer=customer,product=product,quantity=quantity,status='Pending',email=email,mobile=mobile,address=address)
